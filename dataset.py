...
class age_TestDataset(Dataset):
    def __init__(self, img_paths, resize, mean=(0.548, 0.504, 0.479), std=(0.237, 0.247, 0.246)):
        self.img_paths = img_paths
        self.transform = A.Compose([
            A.CenterCrop(384, 256),
            A.Resize(*resize, Image.BILINEAR),
            A.Normalize(mean=mean, std=std),
            ToTensorV2(),
        ])
...
class age_augmentation(Dataset):
...
class Age_only_Dataset(Dataset):
    num_classes = 105
    ...
    def __getitem__(self, index):
        ...
        image = self.read_image(index)
        age_label = self.get_age_label(index)
        class_label = self.encode_class(age_label)
    ...
    def get_age_label(self, index) -> int:
        return self.age_labels[index]
    ...
    @staticmethod
    def encode_class(age_label) -> int:
        return age_label
